...
After downloading the correct executable and correct model (available in Bulgarian, Danish, Estonian, Finnish, Galician, Latin, Mongolian, Polish, Russian, Slovakian and Swahili), we must configure our pipeline in order to be able to use Treetagger. You can find an example configuration in the _configs_ folder _treetagger-example.properties_:
----
